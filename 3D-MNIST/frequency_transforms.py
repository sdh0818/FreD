...
class DCT():
    ...
    def forward(self, x):
        X1 = self.forward_transform(x)  # (bsz, C, D, H, W)
        X2 = self.forward_transform(X1.transpose(-1, -2))  # (bsz, C, D, W, H)
        X3 = self.forward_transform(X2.transpose(-1, -3))  # (bsz, C, H, W, D)
        return X3.permute(0, 1, -1, -3, -2)

    def inverse(self, x):
        X1 = self.inverse_transform(x)  # (bsz, C, D, H, W)
        X2 = self.inverse_transform(X1.transpose(-1, -2))  # (bsz, C, D, W, H)
        X3 = self.inverse_transform(X2.transpose(-1, -3))  # (bsz, C, H, W, D)
        return X3.permute(0, 1, -1, -3, -2)