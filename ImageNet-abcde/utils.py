...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, decay="cosine", return_loss=False, test_it=100, aug=True):
    ...
    save_and_print(args.log_path, "TestAcc Epoch {}:\t{}".format(ep, acc_test))
    if acc_test > acc_test_max:
        acc_test_max = acc_test
        acc_test_max_epoch = ep
        save_and_print(args.log_path, "NewMax {} at epoch {}".format(acc_test_max, acc_test_max_epoch))

    time_train = time.time() - start

    save_and_print(args.log_path, '%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test_max))
    save_and_print(args.log_path, "Max {} at epoch {}".format(acc_test_max, acc_test_max_epoch))
